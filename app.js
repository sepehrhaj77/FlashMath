...
function loadAddition() {
	...
	for (var i = 0; i < MAX_PROBLEMS; i++) {
		num1 = Math.floor(Math.random() * 51) //first number between 0-50
		num2 = Math.floor(Math.random() * 51) //second number between 0-50
		ans = num1 + num2
...
function loadSubtraction() {
	...
	problems = []
	for (var i = 0; i < MAX_PROBLEMS; i++) {
		num1 = Math.floor(Math.random() * 51) + 5 //first number must be between 5-50
...
function helpCheckLeaderboard(cat, score) {
	...
	if (highScores.length < 5) {
		...
		if (highScores.length > 0) {
			//make sure array is not empty so theres no null access exception
